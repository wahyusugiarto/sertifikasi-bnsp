        ...
        <div class="card-content">
          <div class="card-body card-dashboard">
            <div class="table-responsive">
              <?php if ($accessAdd > 0) { ?>
                <a class="klik ajaxify" href="<?php echo site_url('add-kategori'); ?>"><button class="btn btn-icon btn-primary mb-2"><i class="feather icon-plus"></i>&nbsp; Add Data</button></a>
              <?php } ?>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    table = $('#table').DataTable({
      ...
      "ajax": {
        "url": "<?php echo site_url('Section/Kategori/ajax_list') ?>",
        "type": "POST"
      },
  ...
  $(document).on("click", ".hapus-kategori", function() {
    ...
    swal({
      ...
      function() {
        ...
        $.ajax({
          method: "POST",
          url: "<?php echo base_url('Section/Kategori/hapus'); ?>",
          data: "id_kategori=" + id_kategori,
          success: function(data) {
            var result = jQuery.parseJSON(data);
            if (result.status == true) {
              $("tr[data-id='" + id_kategori + "']").fadeOut("fast", function() {
                $(this).remove();
              });
