...
class Supplier extends AUTH_Controller
{
	...
	const __tableId   = 'id';
	const __folder    = 'v_supplier/';
	const __kode_menu = 'kode-supplier';
	const __title     = 'Supplier';
	const __model     = 'M_supplier';
	...
	public function ajax_list()
	{
		...
		foreach ($list as $brand) {
			...
			if ($accessEdit->menuview > 0) {
				$buttonEdit = anchor('edit-supplier/' . $brand->id, ' <span tooltip="Edit Data"><span class="fa fa-edit"></span> ', ' class="btn btn-icon btn-primary klik ajaxify" ');
			}
			$buttonDel = '';
			if ($accessDel->menuview > 0) {
				$buttonDel = '<button class="btn btn-icon btn-danger hapus-supplier" data-id=' . "'" . $brand->id . "'" . '><span tooltip="Delete Data"><i class="fa fa-trash"></i></button>';
			}
	...
	public function prosesTambah()
	{
		...
		if ($errCode == 0) {
			if (strlen($supplier) == 0) {
				$errCode++;
				$errMessage = "supplier wajib di isi.";
			}
		}
	...
	public function Edit($id)
	{
		...
		/*ini harus ada boss */ else {
			...
			$data['dataslider']       = $this->M_supplier->selectById($id);
	...
	public function prosesUpdate()
	{
		...
		if ($errCode == 0) {
			$checkValid = $this->M_supplier->selectById($id);
			if ($checkValid == null) {
				$errCode++;
	...
	public function hapus()
	{
		...
		if ($errCode == 0) {
			$checkValid = $this->M_supplier->selectById($token);
			if ($checkValid == null) {
				$errCode++;
				$errMessage = self::__title . " tidak valid.";
			}
