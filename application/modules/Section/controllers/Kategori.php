...
class Kategori extends AUTH_Controller
{
	...
	const __folder    = 'v_kategori/';
	const __kode_menu = 'kode-kategori';
	const __title     = 'Kategori Barang';
	const __model     = 'M_kategori';
	...
	public function ajax_list()
	{
		...
		foreach ($list as $brand) {
			...
			if ($accessEdit->menuview > 0) {
				$buttonEdit = anchor('edit-kategori/' . $brand->id_kategori, ' <span tooltip="Edit Data"><span class="fa fa-edit"></span> ', ' class="btn btn-icon btn-primary klik ajaxify" ');
			}
			$buttonDel = '';
			if ($accessDel->menuview > 0) {
				$buttonDel = '<button class="btn btn-icon btn-danger hapus-kategori" data-id=' . "'" . $brand->id_kategori . "'" . '><span tooltip="Delete Data"><i class="fa fa-trash"></i></button>';
			}
	...
	public function Edit($id)
	{
		...
		/*ini harus ada boss */ else {
			...
			$data['dataslider']       = $this->M_kategori->selectById($id);
	...
	public function prosesUpdate()
	{
		...
		if ($errCode == 0) {
			$checkValid = $this->M_kategori->selectById($id);
			if ($checkValid == null) {
				$errCode++;
	...
	public function hapus()
	{
		...
		$errCode = 0;
		$errMessage = "";
		$token = $this->input->post('id_kategori');
		$this->db->trans_begin();
		...
		if ($errCode == 0) {
			$checkValid = $this->M_kategori->selectById($token);
			if ($checkValid == null) {
				$errCode++;
				$errMessage = self::__title . " tidak valid.";
			}
